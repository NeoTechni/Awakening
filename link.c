...
const s8 cLinkShadow[4][2]= {	{ 1, 9 }, { 0, 8 }, { -1, 9 }, { 0, 8 }		};
...
void link_move(TSprite *link){
	handleevents	= true;
	if(link->vx != 0){
		link->x 	= can_move_x(link->x, link->y, 0, 1, 3, 2, link->vx, true, true, true);
		//link->x += link->vx;
	}
	if(link->vy != 0){
		link->y		= can_move_y(link->x, link->y, 0, 1, 3, 2, link->vy, true, true, true);
		//link->y += link->vy;
	}
	handleevents	= false;
}
