...
VIEWPORT g_vp = {
	0, 0, 1024, 240, //x, xmin, xmax, xpage;
	0, 0, 1024, 160, //y, ymin, ymax, ypage;
};
...
void bgt_init(TMapInfo *bgt, int bgnr, u32 ctrl, const void *map, u32 map_width, u32 map_height) {
...
void bgt_update(TMapInfo *bgt, VIEWPORT *vp){
	...
	// Basically, we need another row/col when the viewport exposes another row/col, i.e., if the tile coords have changed
...
bool init_map_main(const void *colorpallet, u32 colorpalletLen, const void *tileset, u32 tilesetLen, const void *map, u32 map_width, u32 map_height, u32 link_x, u32 link_y) {
	...
	GRIT_CPY(pal_bg_mem, colorpallet);//		  wid        hei			//color pallet
	bgt_init(&g_bg, 1, BG_CBB(0)|BG_SBB(29), map, map_width, map_height);	//map
...
int map_main() {
	...
	//while(1) {
		...
		map_main_x = g_link.x>>8, map_main_y= g_link.y>>8;
